...
function isContainNumber(num, digit) {
  let decreasedNum = num;
  while (decreasedNum > 0) {
    const curDigit = decreasedNum % 10;
    if (curDigit === digit) {
      return true;
    }
    decreasedNum = Math.floor(decreasedNum / 10);
  }
  return false;
}
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < size; i += 1) {
    for (let j = 0; j < Math.floor(size / 2); j += 1) {
      const temp = newMatrix[i][j];
      newMatrix[i][j] = newMatrix[i][size - j - 1];
      newMatrix[i][size - j - 1] = temp;
    }
  }
...
function sortByAsc(arr) {
  ...
  function quickSort(start, end) {
    ...
    const pivot = newArr[start];
    let left = start + 1;
    let right = end;

    while (left <= right) {
      while (left <= end && newArr[left] < pivot) left += 1;
      while (right > start && newArr[right] >= pivot) right -= 1;
